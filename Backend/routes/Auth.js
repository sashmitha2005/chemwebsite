...
router.post('/adminlogin', async (req, res) => {
  try {
    const { email, password } = req.body;
    console.log("Received data:", { email, password }); // Check if data is correct
...
router.post('/add', adminAuth, async (req, res) => {
  try {
    ...
    const newProduct = new Product({
      adminId: req.userId, // from the token via adminAuth middleware
      name,
      quantity,
      unit,
      price
...
// Place order directly from product view
router.post('/placeorder/:productId', userAuth, async (req, res) => {
  try {
    const { productId } = req.params;
    ...
    // Create the order
    const order = new Order({
      userId: req.userId, // The user placing the order
      items: [orderItem],
...
router.get('/myorders', userAuth, async (req, res) => {
  try {
    console.log("User ID:", req.userId);  // Log user ID to ensure it's being passed correctly

    // Fetch orders and populate user details along with the order details
...
router.get('/allorders', adminAuth, async (req, res) => {
  try {
    const orders = await Order.find()
      ...
      .populate('items.productId', 'name price') // optional: show product name/price
...
router.get("/dashboard", authMiddleware, async (req, res) => {
  try {
    // You can now use req.userId to fetch user-specific data
